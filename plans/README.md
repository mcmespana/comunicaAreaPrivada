...
|------|--------|-----------|----------|------------|--------|
| 001 | Exigir sesión autenticada en los handlers que mutan datos | P0 | S | — | **HECHO** → `archive/` (24/09) — `sticpa_require_session()` en todos; un test recorre los `admin_post_nopriv_*` y falla si alguno no la llama |
| 002 | Eliminar IDOR + mass-assignment en las escrituras al CRM | P0 | M | 001 | **HECHO** → `archive/` (24/09) — el formulario FIRMA la lista de campos que enseña y el handler solo acepta esos (más una lista negra que manda siempre); el `id` sale de la sesión o de la comprobación de propiedad. Pagos y compromisos ya no escriben |
| 003 | Asegurar ver/descargar documentos + validar subidas | P0 | M | 001 | **HECHO** → `archive/` (24/09) — descargar/editar/borrar y las fichas de pago, compromiso, inscripción y documento exigen que el registro salga en TU listado. Subidas: 6 MB y lista de extensiones. `single_stic_sessions` también: la sesión tiene que ser de un evento en el que estás inscrito |
| 004 | Validar el destino del selector de participante | P0 | M | 001 | **HECHO** → `archive/` (24/09) — solo uno mismo o alguien de su lista; el nombre y el familiar salen de la sesión, no del enlace |
| 005 | Corregir el open redirect vía `scp_current_url` | P1 | S | — | **HECHO** → `archive/` (24/09) — `sticpa_return_url()` tira esquema y host; todos los redirects son `wp_safe_redirect` |
| 006 | Escapar los valores del CRM al pintarlos (XSS almacenado) | P1 | M | — | **HECHO** → `archive/` (24/09) — celdas de `makeList`, `readOnly`/`info`/`image`, `<option>` de select y multienum y los radios; `html`/`note` siguen con `wp_kses_post` a propósito |
| 007 | Regenerar el ID de sesión al autenticar (session fixation) | P1 | S | — | **HECHO** → `archive/` (01/09) — adelantado al resto de seguridad por ser de dos líneas y sin riesgo |
| 008 | Endurecer el transporte al CRM: TLS + queries de login | P0 | M | — | **HECHO** → `archive/` (24/09) — TLS verificado (PEER + HOST=2; certificado Let's Encrypt válido); `quoteValue()` en las consultas del login, que admitían `x' OR '1'='1` |
...
| 011 | Eliminar los N+1 de listados, calendario y selector | P1 | L | 013 | **HECHO** → `archive/` (25/09) — la técnica del plan (enlaces anidados) no sirve en esta instancia; cada nivel sale en una tanda paralela (`sticpa_pl_prime`) en calendario/portada, sesiones, asistencias, pagos de menor, guard de inscripción y selector de familia. Y el tope de página del CRM se aprende: cada lista, de 2 viajes a 1 |
| 012 | Sustituir `getAllEmail()` por una consulta puntual | P1 | S | — | **YA NO APLICA** — el alta y `getAllEmail()` se borraron el 10/09/2026 → `archive/` |
...
| 027 | Cliente CRM: keep-alive, HTTP/1.1, timeouts y gzip | P0 | S-M | — (coordina con 008) | **DONE** → `archive/` (`cb79e9c`) |
| 028 | Soltar el lock de sesión PHP (foto/descargas en paralelo) | P1 | S-M | — | **DESCARTADO lo que faltaba** (25/09). Lo que de verdad dolía (la foto y las descargas esperando al HTML) se hizo en `122289f`. Lo pendiente —cerrar la sesión DURANTE el render— perdería en silencio todo lo que se escribe en `$_SESSION` después del cierre, y hoy se escribe mucho mientras se pinta: el rol (`sticpa_get_comunica_role`), los hijos de una familia (`scp_available_profiles`), el aviso del certificado (`scp_ds_pending`), la sesión técnica del CRM y la ventana de la cookie. Un fallo así es invisible (p. ej. «a un monitor le desaparece Pasar lista», que ya pasó con el plan 040). Ganancia moderada, riesgo alto. Si algún día se retoma: primero un censo por grep de cada `$_SESSION[...] =` alcanzable desde el render → `archive/` |
| 029 | Quick wins de lecturas CRM (guard, caches, typo de pago) | P1 | M | — | **DONE** → `archive/` (`967ef24`) |
| 030 | Tap instantáneo: feedback en enlaces + prefetch + puente app | P1 | S-M | — | **DONE** → `archive/` (`d2873dd` + prefetch 2026-08-13) |
| 031 | Dieta de assets: CSS minificado en deploy + Inter local | P2 | M | — | **HECHO** → `archive/` (`7dc928a` + 25/09: DataTables ya no se carga en los listados; `makeList()` la pide si vuelve a usarse) |
| 032 | Acotar listados: ventana de eventos + techo de filas | P2 | M | — | **FUTURO** (25/09). La ventana de eventos está hecha (`113255e`). El techo de filas exige pedir al CRM las más recientes con un `order_by` por módulo que no está verificado, y hoy las listas de cada persona son cortas (decenas, no cientos). Retomarlo cuando alguna crezca de verdad |
| 033 | Pasar Lista: guardado visible y cierre de EL bug | **P0** | M | — | **HECHO** → `archive/` — confirmado en producción el 27/08/2026 |
| 034 | Pasar Lista: rendimiento (calentar, paralelizar, medir) | P1 | M | 033 | **HECHO** → `archive/` — 1+N de monitores muerto y `curl_multi` en las cinco pantallas (viajes: 7→3, 6→2, 8→4, 7→3, 9→4). Falta la foto real de producción |
| 035 | Pasar Lista: fidelidad al diseño de los artboards | P2 | M | — | **HECHO** → `archive/` (28/08) — las seis pantallas con artboard, verificadas con captura en los dos temas. Tres fallos de fondo, todos del tema de WordPress ganando por especificidad: los `<label>` sin su `display:flex`, los `<input>` con caja dentro de la caja, y 32 px de scroll lateral |
| 036 | Pasar Lista: funcionalidad pendiente + doctrina de la API | P2 | por fila | 033 | **CASI TODO HECHO** (25/09): Najar, grupos viejos (casilla `ajmcm_pasar_lista_c`) y recuentos, hechos y comprobados en producción. Quedan dos cosas fuera del código: el correo de avisos (config. del CRM) y verificar `CAMPOS.md` contra el CRM (necesita el conector MCP de SinergiaCRM) |
| 037 | Pasar Lista: UX — fluidez, menos toques, ficha a un gesto | P1-P2 | S-M por fila | 033 | **HECHO** (25/09): filas 1, 3 y 5 hechas; la 2 descartada por el propietario; la 4 obsoleta (el overlay al tocar ya existe); la 6 cubierta por la vista A-Z de Mis grupos → `archive/` |
| 040 | Rol de Comunica: no cachear un rol que no se ha podido resolver | P1 | S | — | **HECHO** → `archive/` (01/09) — a un monitor le desaparecieron «Pasar lista» y «Mis grupos» en producción |
| 038 | Pasar Lista: seguimiento de monitores (asistencia y listas) | P2 | M | 033, paginación | **HECHO** → `archive/` (28/08/2026) — tres pistas en la ficha del monitor y el aviso mínimo en la lista. Con tres cambios de diseño sobre lo escrito, en §7 del plan |
| 039 | Unificar el lenguaje visual MCM (tokens, breakpoints, deuda de diseño) | P2 | S por fila | — | **HECHO** → `archive/` (31/08) — las 9 filas. Tokens semánticos en las dos superficies, escala de breakpoints, secciones de CSS sin duplicados, CAMPOS.md sincronizado solo e Inter autoalojada también en los formularios. Sale de escribir [`../design.md`](../design.md) |
